$(document).ready(function () {
  ...
  $(document).on("submit", "#playlist_form", function (e) {
    ...
    $("#single_video_table").append(
      "<tr><th>Sl No</th><th>Thumbnail</th><th>Title</th><th>Download</th></tr>",
    );
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (json_data.error != "") {
          ...
          $("#single_video_thumbnail_id").html(
            '<img src="' +
              json_data.thumbnail +
          ...
          var streams =
            "<select id='single_download_select_id' onchange='optionChanged()'><script type='text/javascript'> function optionChanged(){$('#dynamicURL').attr('href', $('#single_download_select_id :selected').val());}</script>";
          for (i = 0; i < Object.keys(data.streams).length; i++) {
            streams +=
              "<option value='" +
              Object.values(json_data.streams)[i] +
              "'>" +
              Object.keys(json_data.streams)[i] +
              "</option>";
          }
          streams +=
            "</select><a id='dynamicURL' href=''><button class='single-download-button'>Download</button></a>";
